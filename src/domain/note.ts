...
export function fromMidi(midi: MidiValue): Note {
  ...
  const octave = Math.floor(pianoRange / SEMITONES_IN_OCTAVE) as OctaveIndex;

  const index = pianoRange % SEMITONES_IN_OCTAVE;
  const pitch = PITCHES_REGISTRY[index];

  const isShape = !NATURAL_PITCH_INDICES.includes(index);
  const type = isShape ? "sharp" : "natural";
